...
int find_proc(int pid)
{
	...
	for(i=0; i<MAX_PROC; i++)
	{
		mutex_lock(&proc_data[i].mt);
		if(proc_data[i].pid == pid)
		{ mutex_unlock(&proc_data[i].mt); return i;}
		mutex_unlock(&proc_data[i].mt);
	}
	return -1;
}
...
int device_open(struct inode* inode, struct file* filp)
{
	...
	printk(KERN_INFO "partb_A2: device opened\n");
	printk(KERN_INFO "partb_A2: The process id is %d and the index is %d\n", pid, id);
	return 0;
} 
...
int device_close(struct inode* inode, struct file* filp)
{
	...
	printk(KERN_INFO "partb_A2: closing device for process %d\n", pid);
	return 0;
}
...
ssize_t device_read(struct file *filp, char* bufStoreData, size_t bufsize, loff_t* curroffset)
{
	...
	if(proc_data[id].tm == 'z')
		return -EACCES;
	printk(KERN_INFO "partb_A2: reading data for process %d\n", pid);
	if(proc_data[id].tm == 'i')
	{
		if(proc_data[id].mode == 0)
		{
			if(proc_data[id].curr_read == 0)
			{
				...
				printk(KERN_INFO "partb_A2: inorder done for process %d\n", pid);
			}
			if(proc_data[id].head_it == NULL)
			{
				printk(KERN_ALERT "partb_A2: No more data to read for process %d\n", pid);
				return 0;
			}
			printk(KERN_INFO "partb_A2: value read is %d for process %d\n", (proc_data[id].head_it)->node, pid);
		...
		else if(proc_data[id].mode == 1)
		{
			...
			printk(KERN_INFO "partb_A2: value read is %s for process %d\n", (proc_data[id].head_ct)->node, pid);
	...
	else if(proc_data[id].tm == 'p')
	{
		if(proc_data[id].mode == 0)
		{
			if(proc_data[id].curr_read == 0)
			{
				...
				printk(KERN_INFO "partb_A2: preorder done for process %d\n", pid);
			}
			if(proc_data[id].head_it == NULL)
			{
				printk(KERN_ALERT "partb_A2: No more data to read for process %d\n", pid);
	...
	else if(proc_data[id].tm == 's')
	{
		if(proc_data[id].mode == 0)
		{
			if(proc_data[id].curr_read == 0)
			{
				...
				printk(KERN_INFO "partb_A2: postorder done for process %d\n", pid);
			}
			if(proc_data[id].head_it == NULL)
			{
				printk(KERN_ALERT "partb_A2: No more data to read for process %d\n", pid);
...
ssize_t device_write(struct file *filp, const char* bufSourceData, size_t bufsize, loff_t* curroffset)
{
	...
	proc_data[id].head_it = NULL;
	proc_data[id].head_ct = NULL;
	printk(KERN_INFO "partb_A2: node insertion comlete for process %d\n", pid);
	return ret;
}
	...
	switch(cmd)
	{
		case PB2_SET_TYPE:
						...
						if(t==0xFF)
							...
							return -EINVAL;
						printk(KERN_INFO "partb_A2: Mode changed to mode %c for process %d \n", t, pid);
						return 0;
		...
		case PB2_SET_ORDER:
						...
						if(proc_data[id].mode == 0)
							...
							proc_data[id].head_ct = NULL;
						proc_data[id].curr_read = 0;
						printk(KERN_INFO "partb_A2: Traversal order set to %c for process %d\n", tm, pid);
		...
		case PB2_GET_INFO:
						...
						printk(KERN_INFO "partb_A2: info calculated for process %d\n", pid);
		...
		case PB2_GET_OBJ:
						...
						printk(KERN_INFO "partb_A2: Object find completed for process %d\n", pid);
						return 0;
...
static int driver_entry(void)
{
	...
	initialize_data();
	printk(KERN_INFO "partb_A2: allocated partb_A2 entry");
	return 0;
}
...
static void driver_exit(void)
{
	...
	printk(KERN_ALERT "partb_A2: device unloaded\n");			
}
